namespace Piano
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("Переключение между октавами - от 1 и до 6");
        ...
        static int[] Octave(int[] passedOctave, string textOctave)
        {
            ...
            while (true)
            {
                ...
                if (Button.Key == ConsoleKey.F2 || Button.Key == ConsoleKey.F3 || Button.Key == ConsoleKey.F4 || Button.Key == ConsoleKey.F5 || Button.Key == ConsoleKey.F6 || Button.Key == ConsoleKey.F7 || Button.Key == ConsoleKey.F8 || Button.Key == ConsoleKey.F9) ChangeOfOctaves();
                Console.Clear();
